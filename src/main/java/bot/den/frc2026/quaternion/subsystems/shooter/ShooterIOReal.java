...
public class ShooterIOReal implements ShooterIO, CanBeAnInstrument {
    ...
    private final TalonFXS feeder = new TalonFXS(ShooterConstants.FEEDER_MOTOR_ID);
    ...
    public ShooterIOReal() {
        ...
        var feederConfig = new TalonFXSConfiguration()
                .withCurrentLimits(
                        new CurrentLimitsConfigs()
                                .withStatorCurrentLimitEnable(true)
                                .withStatorCurrentLimit(60))
        ...
        BaseStatusSignal.setUpdateFrequencyForAll(
                feeder.getIsProLicensed().getValue() ? 200 : 50,
        ...
        ParentDevice.optimizeBusUtilizationForAll(shooter, feeder, hood);
    }

    @Override
    public void updateInputs(ShooterIOInputs inputs) {
        var shooterStatus = BaseStatusSignal.refreshAll(shooterVelocityRotPerSec, shooterCurrentAmps);
        var feederStatus = BaseStatusSignal.refreshAll(feederVelocityRotPerSec, feederCurrentAmps);
    ...
    public void addInstruments(Orchestra orchestra) {
        ...
        orchestra.addInstrument(feeder);
    ...
    public void setFeederVelocity(AngularVelocity velocity) {
        feeder.setControl(feederSpin.withVelocity(velocity.in(RotationsPerSecond)));
    }
